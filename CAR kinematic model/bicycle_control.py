...
class Car_Dynamics:
    def __init__(self, x_0, y_0, v_0, psi_0, delta, length, dt, a):
        ...
        self.psi = psi_0
        self.delta = delta
        self.state = np.array([[self.x, self.y, self.v, self.psi, self.delta]]).T
        self.a = a
        self.b = length - a

    def move(self, accelerate, delta_dot):
        self.beta = np.arctan((self.b * np.tan(self.delta)) / (self.L))
        x_dot = self.v*np.cos(self.psi + self.beta)
        y_dot = self.v*np.sin(self.psi + self.beta)
        v_dot = accelerate
        psi_dot = self.v*np.tan(self.delta)*np.cos(self.beta)/self.L
        delta_dot = delta_dot
        return np.array([[x_dot, y_dot, v_dot, psi_dot, delta_dot]]).T
